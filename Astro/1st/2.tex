\documentclass[11pt,a4paper]{scrartcl}
\usepackage[top=2cm,bottom=5.5cm,left=2cm,right=2cm]{geometry}
\usepackage{fontspec}
\usepackage{polyglossia}
    \setdefaultlanguage{german}
\usepackage{fancyhdr}
\usepackage{csquotes}
\usepackage{enumitem}
\usepackage{mathtools}
\usepackage{amssymb}
\usepackage{amsfonts}
\usepackage{siunitx}
    \sisetup{%
        range-units=brackets,
        range-phrase={ bis },
    }
    \DeclareSIUnit{\jansky}{Jy}
\usepackage{physics}
\usepackage{wasysym}
\usepackage{booktabs}
\usepackage[%
    labelformat=simple,
    labelsep=none,
    textformat=none,
    font={small,sc}
]{caption}
\usepackage{graphicx}
    \graphicspath{img}
\usepackage{pgfplots}
    \pgfplotsset{%
        table/search path={data},
        label style={font=\tiny},
        tick label style={font=\tiny}
    }
\usepackage[disable]{todonotes}
\usepackage[%
    % colorlinks=true, linkcolor=blue,
    hidelinks
]{hyperref}


\newcommand{\tablehead}[1]{\multicolumn{1}{c}{#1}}
\newcommand*{\figref}[1]{(siehe Abb.~\ref{#1})}
\newcommand{\zB}{z.\,B.}


\newcommand{\course}{\textbf{Einführung in die Astrophysik}}
\newcommand{\hwnumber}{1}
\newcommand{\nameA}{Jeremiah Lübke}
\newcommand{\nameB}{Andreas Menzel}
\newcommand{\matnumA}{108015230366}
\newcommand{\matnumB}{108015226385}
\newcommand{\groupnum}{Übungsgruppe 3}


\pagestyle{fancyplain}
\headheight 7\baselineskip
\lhead{%
    \course \\
    \vspace*{3\baselineskip}
    \nameA, \matnumA \\
    \nameB, \matnumB
}

\chead{%
    \textbf{\Large Hausaufgabenblatt \hwnumber} \\
    \vspace*{2\baselineskip}
}

\rhead{%
    \today \\
    \vspace*{4\baselineskip}
    \groupnum
}

\cfoot{\small\thepage}
\headsep 1.5em


\DeclareSIUnit{\erg}{erg}
\DeclareSIUnit{\mag}{mag}
\newcommand{\Fnu}{\ensuremath{{F}_{\nu}}}
\newcommand{\Inu}{\ensuremath{{I}_{\nu}}}
\newcommand{\Mab}{\ensuremath{m_{\mathrm{AB}}}}
\newcommand{\Pow}{\ensuremath{{P}}}
\newcommand{\Area}{\ensuremath{{A}}}

\newcommand{\cgsflux}{\erg\per\second\per\centi\metre\squared\per\hertz}


\begin{document}

\section*{Aufgabe 2}

Definition der AB-Magnitude: $\Mab = -2.5\,\log(\Fnu) - 48.60$

\begin{enumerate}[label=\textbf{\large(\alph*)}, itemsep=2\baselineskip]

\item
    Strahlungsflussdichte bei nullter AB-Magnitude:
    \begin{equation*}
        \implies \eval{\Fnu}_{\Mab=0} =
        \eval{10^{\frac{\Mab+48.60}{-2.5}}}_{\Mab=0} \approx
        \SI{36.31e-21}{\cgsflux} = \SI{3631}{\jansky}
    \end{equation*}


\item
    Betrachte im Abstand von \SI{2}{\metre} eine Lampe mit
    \begin{itemize}
        \item $P=\SI{2}{\watt}$ Strahlungsleistung
        \item Konstantem Spektrum für Wellenlängen in
            \SIrange[range-units=repeat]{30e3}{300}{\nano\metre}
            $\implies \Delta\nu=\SI{297e-7}{\metre}$
    \end{itemize}
    \medskip
    Die Strahlungsflussdichte kann geschrieben werden als:
    \begin{align*}
        \Fnu &= \int_{\mathrm{Halbraum}}\Inu\dd{\Omega} \\
        \Inu &\propto \frac{\dd\Pow}{\dd\Area\,\dd\Omega\,\dd\nu}
        \overset{\mathrm{hier}}{\propto}
        \dv{\left(\frac{\Pow}{\Area\,\Delta\nu}\right)}{\Omega} \\
        &\implies \Fnu = \frac{\Pow}{\Area\,\Delta\nu}
    \end{align*}
    \todo[inline]{Ist die Gleichung für \Fnu~so korrekt?}

    und mit $\Area = \Area_\mathrm{Halbraum} = 2\pi\,r^2$:
    \begin{align*}
        &\Fnu(r=\SI{2}{\metre}) = \SI{2.68e6}{\cgsflux} = \SI{268e27}{\jansky} \\
        &\implies \Mab = \SI{-64.67}{\mag}
    \end{align*}

    \todo[inline]{Sinnvoller Wert? Interpretation und Vergleich!}


\item
    Die Lampe befinde sich nun auf dem Mond (Abstand \SI{385000}{\kilo\metre}):
    \begin{align*}
        &\Fnu(r=\SI{385e6}{\metre}) =
        \SI{72.31e-12}{\cgsflux} = \SI{7231e9}{\jansky}\\
        &\implies \Mab = \SI{-23.25}{\mag}
    \end{align*}

    \todo[inline]{Sinvoller Wert? Mir erscheint er sehr groß für eine 2W-Lampe auf dem
    Mond\ldots~Es ist aber immerhin weniger als der Vollmond}

\end{enumerate}

\subsection*{Diskussion}
Die Ergebnisse in \textbf{(b)} und \textbf{(c)} sind nach intuitivem
Verständnis viel zu hoch. Vermutlich ist ein Fehler in unserem Ansatz sein.


\end{document}
