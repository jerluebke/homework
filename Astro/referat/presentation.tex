...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \only<5->{%
        \item
            Working out \eqref{eq:b} in cylindrical coordinates, with
            $\uvec=r\,\Omega\,\underline{e}_{\phi}$ and some
            galaxy-related assumptions, one finally arrives at:

            \only<5>{\vspace{9.4\baselineskip}}
            \only<6-7>{%
            \begin{equation}
                \mathcal{E}=0\implies
                \left\{\begin{aligned}
            ...
            \only<8-9>{\vspace{3.6\baselineskip}}
            \only<10->{%
            \vspace{\baselineskip}
            \begin{columns}
            \column{.35\textwidth}
